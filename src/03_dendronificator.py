...
taxo_folder = input_df.apply(lambda x: rootdir /
                                 ...
                                 str(x['organism_otol_family']) /
                                 str(x['organism_otol_genus']) /
                                 f"{x['organism_otol_species']}.md", axis='columns')
...
for csvfile, data in input_df.groupby(taxo_folder):
    csvfile.parent.mkdir(parents=True, exist_ok=True)
    # data.to_csv(csvfile, index=False)
    if math.isnan(data['organism_otol_species_ott_id']) != True:
        ott_id = str(int(data['organism_otol_species_ott_id'].values))
    else:
        ott_id = data['organism_otol_species_ott_id']
    unique_name = str(
        data['organism_otol_species'].values[0]).replace(' ', '_')
    print(ott_id)
    print(unique_name)
    # print(data['ott_id'])
    # with open(csvfile, "w") as text_file:
